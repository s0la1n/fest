...
export default function OrganizerCosplayApplicationsPage() {
  ...
  useEffect(() => {
    if (user && hasRole('cosplay_organizer')) {
      apiClient.get<any[]>('/organizer/cosplay/applications').then((d: any) => setList(d.data ?? d ?? [])).catch(() => setList([])).finally(() => setLoading(false));
    } else {
      setLoading(false);
    }
  }, [user, hasRole]);
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-200 py-12 px-4">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl font-bold text-white mb-8">Заявки на конкурс косплея</h1>
        {loading ? (
          ...
          <ul className="space-y-4">
            {list.map((app: any) => (
              <li key={app.id} className="bg-slate-800 rounded-lg p-4 border border-slate-700 flex justify-between items-center">
                <span className="text-white font-medium">{app.character_name ?? `Заявка #${app.id}`}</span>
